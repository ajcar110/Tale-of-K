...
onready var heldpoint = $Sprite/HeldPoint
...
func fire_bullet():
	var bullet = Utils.instance_scene_on_main(PlayerBullet, heldpoint.global_position)
	bullet.velocity = Vector2(BULLET_SPEED,0)
	bullet.velocity.x *= sprite.scale.x
	fireBulletTimer.start()

func fire_missile():
	var missile = Utils.instance_scene_on_main(PlayerMissile, heldpoint.global_position)
	missile.velocity =Vector2(MISSILE_BULLET_SPEED,0)
	missile.velocity.y = missile.Projectile_velocity
...
func update_animations(input_vector):
	var facing = sign(get_input_vector().x)
	if facing != 0:
